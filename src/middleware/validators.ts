...
export const nameValidation = body('name')
    ...
    .isLength({ min: 1 , max: 15}).withMessage('Name must be longer than 1 and not longer than 15')

export const descriptionValidation = body('description')
    .exists().withMessage('Description is required')
    .isString().withMessage('Description must be a string').trim()
    .isLength({ min: 1 , max: 500}).withMessage('Description must be longer than 1 and not longer than 500')

export const webUrlValidation = body('websiteUrl')
    .exists().withMessage('WebsiteUrl is required')
    .isString().withMessage('WebsiteUrl must be a string').trim()
    .isLength({ min: 1 , max: 100}).withMessage('WebsiteUrl must be longer than 1 and not longer than 100')
    .isURL().withMessage('WebsiteUrl is not a valid URL')

export const titleValidation = body('title')
    .exists().withMessage('Title is required')
    .isString().withMessage('Title must be a string').trim()
    .isLength({ min: 1 , max: 30}).withMessage('Title must be longer than 1 and not longer than 30')

export const shortDescValidation = body('shortDescription')
    .exists().withMessage('ShortDescription is required')
    .isString().withMessage('ShortDescription must be a string').trim()
    .isLength({ min: 1 , max: 100}).withMessage('ShortDescription must be longer than 1 and not longer than 100')

export const contentValidation = body('content')
    .exists().withMessage('Content is required')
    .isString().withMessage('Content must be a string').trim()
    .isLength({ min: 1 , max: 1000}).withMessage('Content must be longer than 1 and not longer than 1000')

export const blogIdValidation = body('blogId')
    .exists().withMessage('BlogId is required')
    .isString().withMessage('BlogId must be a string').trim()
